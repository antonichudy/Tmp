...
ca_density <- ggplot(data = ca) +
  geom_polygon(aes(x = long, y = lat, group = group,
                   fill = people_per_sq_mile),
               color = "white") +
  coord_map("albers", 25, 50) +
  scale_fill_viridis_c(option = "plasma", name = "Liczba osób na milę²") +
  theme_minimal()
...
world$indicator <- rnorm(length(unique(world$region)), mean = 0, sd = 5)[match(world$region, unique(world$region))]

w1_prim <- world %>% filter(long <= 180)

ggplot(data = w1_prim) +
  geom_polygon(aes(x = long, y = lat, group = group, fill = indicator),
               color = "gray20", size = 0.2) +
  coord_map("mollweide") +
  scale_fill_gradient2(low = "blue", mid = "white", high = "red",
                       midpoint = 0, name = "Wskaźnik") +
  theme_void()
...
ggplot(data = granice_powiatow) + geom_sf()
ggplot(data = granice_powiatow) + geom_sf(color = "pink", fill = "blue")
ggplot(data = granice_powiatow) + geom_sf(color = "black") + theme_void()
...
wojewodztwa$JPT_NAZWA_ <- tolower(wojewodztwa$JPT_NAZWA_)
dane$Nazwa <- tolower(dane$Nazwa)
wojewodztwa2 <- wojewodztwa %>%
  left_join(dane, by = c("JPT_NAZWA_" = "Nazwa"))

colnames(wojewodztwa2)

#wojewodztwa$zdawalnosc <- gsub(",", ".", wojewodztwa$zdawalnosc)
wojewodztwa2$zdawalnosc <- as.numeric(wojewodztwa2$ogółem.2024....)
summary(wojewodztwa2$zdawalnosc)

ggplot(data = wojewodztwa2) +
  geom_sf(aes(fill = zdawalnosc), color = "white") +
  scale_fill_viridis_c(option = "magma", name = "Zdawalność (%)") +
  theme_void() +
  labs(title = "Zdawalność egzaminu maturalnego w Polsce")
