...
plot_ly(df, x = ~HP, type = "histogram", nbins = 30)
...
# Dane dla Pikachu
pikachu_stats <- c(35, 55, 40, 50, 50, 90)
pikachu_closed <- c(pikachu_stats, pikachu_stats[1])
# Dane dla Raichu
raichu_stats <- c(60, 90, 55, 90, 80, 110)
raichu_closed <- c(raichu_stats, raichu_stats[1])
# Tworzenie wykresu
fig <- plot_ly(type = 'scatterpolar', mode = 'markers+lines') %>%
  ...
  add_trace(
    ...
    fill = 'toself',
    name = 'Pikachu',
    fillcolor = 'rgba(255, 255, 0, 0.3)', # Żółty z przezroczystością
  ...
  add_trace(
    ...
    fill = 'toself',
    name = 'Raichu',
    fillcolor = 'rgba(255, 165, 0, 0.3)', # Pomarańczowy z przezroczystością
  ...
  layout(
    polar = list(
      radialaxis = list(
        visible = TRUE,
        range = c(0, 120) # Maksymalna statystyka Raichu to 110
      )
    ),
    title = "Porównanie statystyk: Pikachu vs Raichu",
    showlegend = TRUE
  )
fig
