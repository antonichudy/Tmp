...
ggplot()
#Wywołanie bez argumentów tworzy 
#pusty obiekt ggplot — niczego nie wyrysuje.
#Przydaje się jako punkt startowy.


ggplot(data = countries)
#Tworzy obiekt ggplot z ustawionym źródłem danych
#countries. Nadal nie ma mapowań osi ani geometrii,
# więc wykres będzie pusty, ale obiekt pamięta dataset.


## 2. Mapowania ----> aes()

ggplot(data = countries,
       mapping = aes(x = birth.rate, y = death.rate))
#Tworzy obiekt ggplot z mapowaniem estetyk (aes):
#oś x ← birth.rate, oś y ← death.rate. Nadal bez warstwy
# geometrycznej wykres nic nie pokaże.
...
ggplot(data = countries, 
       mapping = aes(x = birth.rate, y = death.rate)) +
  geom_point() 
#geom_point() — rysuje punkty (scatterplot). Teraz zobaczysz
#rozrzut wskaźnika urodzeń względem wskaźnika zgonów
# dla krajów.
...
#djust zmienia szerokość jądra (mniejsze → bardziej szczegółowo, większe → wygładzanie).
#alpha = 0.7 wypełnia obszar półprzezroczystym kolorem.
...
countries %>%
  group_by(continent) %>%
  filter(
    population > quantile(population, 0.25, na.rm = TRUE) - 1.5 * IQR(population, na.rm = TRUE),
    population < quantile(population, 0.75, na.rm = TRUE) + 1.5 * IQR(population, na.rm = TRUE)
  ) %>%  ggplot(aes(y = population, x = continent)) +
  geom_boxplot()
...
# Zadanie 2
# Narysuj wykres gęstości wskaźnika śmierci,
# zadbaj o czytelność wykresu.
