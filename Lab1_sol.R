...
res <- sapply(
  top1000passwords,
  function(p) proton(action = "login", login = "johnins", password = p)
)

# wybieramy hasło/y które dały sukces
success_pw <- top1000passwords[res == "Success! User is logged in!"]
print(success_pw)




# zwracamy odpowiedź serwera dla każdego hasła
res <- vapply(top1000passwords,
              FUN = function(p) proton(action = "login", login = "johnins", password = p),
